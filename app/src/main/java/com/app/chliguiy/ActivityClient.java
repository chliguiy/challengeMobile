...
public class ActivityClient extends AppCompatActivity {
    ...
    private Call<CallbackClient> callbackCall = null;
...
///                requestAction(1);
    ...
    private void displayApiResult(final List<Client> items) {

        SimpleAdapter adapt = new SimpleAdapter (getApplicationContext(), list_Client, R.layout.item_client,
                new String[] {"code", "solde","total_ca","total_regl"}, new int[] {R.id.code, R.id.solde, R.id.ca,R.id.cr});//R.id.txt_four
        //Log.i("Set adapter", "");
        LayoutInflater factory = LayoutInflater.from(this);
        View myView = factory.inflate(R.layout.item_clientheader, null);
    ...
    private void requestListNewsInfo(final int page_no) {
        ...
        if(id==null){id=null;}
        callbackCall = api.getListClient(page_no, Constant.NEWS_PER_REQUEST, null,id);
        callbackCall.enqueue(new Callback<CallbackClient>() {
            @Override
            public void onResponse(Call<CallbackClient> call, Response<CallbackClient> response) {
                CallbackClient resp = response.body();
                if (resp != null && resp.status.equals("success")) {
                    ...
                    for(int i=0;i<post_total;i++){
                        ...
                        map.put("code",resp.client.get(i).code);
                        map.put("solde",resp.client.get(i).Solde);
                        map.put("total_ca",resp.client.get(i).total_ca);
                        map.put("total_regl",resp.client.get(i).total_reglement);
                        list_Client.add(map);
                    }

                    displayApiResult(resp.client);
                } else {
            ...
            @Override
            public void onFailure(Call<CallbackClient> call, Throwable t) {
