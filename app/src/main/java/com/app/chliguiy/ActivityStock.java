package com.app.chliguiy;
        ...
        import com.app.chliguiy.adapter.AdapterNewsInfo;
...
public class ActivityStock extends AppCompatActivity {
    ...
    private Call<CallbackStock> callbackCall = null;
    ...
    private void initToolbar() {
        ...
        actionBar.setTitle(R.string.title_activity_stock);
    ...
    private void displayApiResult(final List<Stock> items) {

        SimpleAdapter adapt = new SimpleAdapter (getApplicationContext(), list_Client, R.layout.item_stock,
                new String[] {"famille", "article","pin","stock"}, new int[] {R.id.famille, R.id.article, R.id.pin,R.id.stock});//R.id.txt_four
        //Log.i("Set adapter", "");
        LayoutInflater factory = LayoutInflater.from(this);
        View myView = factory.inflate(R.layout.item_stockheader, null);
    ...
    private void requestListNewsInfo(final int page_no) {
        ...
        if(id==null){id=null;}
        callbackCall = api.getlistStock(page_no, Constant.NEWS_PER_REQUEST, null,id);
        callbackCall.enqueue(new Callback<CallbackStock>() {
            @Override
            public void onResponse(Call<CallbackStock> call, Response<CallbackStock> response) {
                CallbackStock resp = response.body();
                if (resp != null && resp.status.equals("success")) {
                    ...
                    for(int i=0;i<post_total;i++){
                        ...
                        map.put("famille",resp.stock.get(i).famille);
                        map.put("pin",resp.stock.get(i).pin);
                        map.put("article",resp.stock.get(i).article);
                        map.put("stock",resp.stock.get(i).stock);
                        list_Client.add(map);
                    }

                    displayApiResult(resp.stock);
                } else {
            ...
            @Override
            public void onFailure(Call<CallbackStock> call, Throwable t) {
