...
public class activityRapportList  extends AppCompatActivity {
...
///                requestAction(1);
    ...
    private void displayApiResult(final List<rapport> items) {
recyclerView.destroyDrawingCache();
recyclerView.removeAllViewsInLayout();

        SimpleAdapter adapt = new SimpleAdapter (getApplicationContext(), list_Client, R.layout.item_rapport_header,
                new String[] {"date", "description"}, new int[] {R.id.date, R.id.description});//R.id.txt_four
        //Log.i("Set adapter", "");
    ...
    private void requestListNewsInfo(final int page_no) {
        ...
        if(id==null){id=null;}
        callbackCall = api.getListrapport(page_no, Constant.NEWS_PER_REQUEST, null,id);
            ...
            public void onResponse(Call<CallbackRapport> call, Response<CallbackRapport> response) {
                ...
                if (resp != null && resp.status.equals("success")) {
                    ...
                    for(int i=0;i<post_total;i++){
                        ...
                        map.put("description",resp.rapport.get(i).description);
                        map.put("date",resp.rapport.get(i).date);
                        list_Client.add(map);
                    }
                    displayApiResult(resp.rapport);
                } else {
