...
public class ActivityFraisList extends AppCompatActivity {
    ...
    private Call<CallbackFrais> callbackCall = null;
...
///                requestAction(1);
    ...
    private void displayApiResult(final List<Frais> items) {

        SimpleAdapter adapt = new SimpleAdapter (getApplicationContext(), list_Client, R.layout.item_frais,
                new String[] {"date", "ville","depart","retour","hotel","repas","parking","remarque"}, new int[] {R.id.date, R.id.ville, R.id.depart,R.id.retour,R.id.hotel,R.id.repas,R.id.parking,R.id.remarque});//R.id.txt_four
        //Log.i("Set adapter", "");
        LayoutInflater factory = LayoutInflater.from(this);
        View myView = factory.inflate(R.layout.item_frais_header, null);
    ...
    private void requestListNewsInfo(final int page_no) {
        ...
        list_Client.clear();
        if(id==null){id=null;}
        callbackCall = api.getListFrais(page_no, Constant.NEWS_PER_REQUEST, null,id);
        callbackCall.enqueue(new Callback<CallbackFrais>() {
            @Override
            public void onResponse(Call<CallbackFrais> call, Response<CallbackFrais> response) {
                CallbackFrais resp = response.body();
                if (resp != null && resp.status.equals("success")) {
                    post_total = resp.count_total;
                    for(int i=0;i<post_total;i++){
                        HashMap<String, String> map = new HashMap<String, String>();
                        map.put("date",resp.frais.get(i).date);
                        map.put("ville",resp.frais.get(i).ville);
                        map.put("depart",resp.frais.get(i).depart_km);
                        map.put("retour",resp.frais.get(i).retour_km);
                        map.put("hotel",resp.frais.get(i).hotel);
                        map.put("repas",resp.frais.get(i).repas);
                        map.put("parking",resp.frais.get(i).parking_peage);
                        map.put("remarque",resp.frais.get(i).remarque);
                        list_Client.add(map);
                    }

                    displayApiResult(resp.frais);
                } else {
            ...
            @Override
            public void onFailure(Call<CallbackFrais> call, Throwable t) {
